...
ENV HERMES_WRITE_SAFE_ROOT=/opt/data/discovery-scout
ENV PATH=/opt/hermes/.venv/bin:/opt/data/bin:/opt/data/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
ENV HERMES_CMD=/opt/hermes/.venv/bin/hermes
ENV PORT=8765

WORKDIR /opt/data/discovery-scout
USER hermes
EXPOSE 8765
ENTRYPOINT ["/opt/hermes/.venv/bin/python", "/opt/discovery-runtime/discovery_runtime.py"]
