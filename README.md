...
- **Perplexity:** fixed Search API and current `/v1/sonar` endpoints with bounded search results/citations and models `sonar`, `sonar-pro`, `sonar-reasoning-pro`, and `sonar-deep-research`. Managed full discovery research uses `sonar-deep-research` with a 20-minute provider timeout. The parent tries the direct Perplexity API first and, only when that exact deep-research call is unavailable, quota/entitlement-limited, rate-limited, or transiently failing, retries the same `perplexity/sonar-deep-research` model through OpenRouter. Invalid-request errors fail closed rather than falling back.
